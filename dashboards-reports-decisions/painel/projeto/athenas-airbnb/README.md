...
- **Fonte de Dados**: [athens-data.csv](./data/athens-data.csv)
...
### 3. Resumo de KPIs (Scorecard)
- **Objetivo**: Fornecer números rápidos sobre o portfólio total.
- **Configuração**:
    - **Medidas**: `AVG(Price)` e `COUNT(IDS)`.
    - **Tipo**: Cartões de Texto (KPI Scorecards).
- **Função**: Serve como o "gancho" inicial do painel, dando a escala do mercado antes do detalhamento geográfico.

![Dashboard Final: KPIs, Mapa e Gráfico de Barras](assets/athens_dashboard_final.png)

---

## ✅ Critérios de Conclusão Atendidos

Para garantir a qualidade e utilidade do painel, os seguintes critérios foram validados:

*   **Múltiplas Visualizações**: O painel inclui 3 tipos de visualizações (Barras, Mapa e KPIs) para responder às perguntas de negócio.
*   **Uso de Gráficos Prévios**: Integra o trabalho de análise realizado nas etapas anteriores.
*   **Hierarquia Visual**: O layout prioriza os KPIs e o Preço Médio no topo/esquerda, seguindo o padrão de leitura ocidental.
*   **Interatividade**: Implementação de filtros de ação para exploração dinâmica.
